module UsersHelper
	...
	def friend_buttons(user)
		case current_user.friendship_status(user)
			...
			when "requested"
				accept = link_to "Deny", friendship_path(current_user.friendship_relation(user)), method: :delete
				deny = link_to "Accept", accept_friendship_path(current_user.friendship_relation(user)), method: :put
				return accept + " | " + deny
		end
	end
